...
public class OpenDataJob extends QuartzJobBean{
	
	protected Logger logger = LoggerFactory.getLogger(OpenDataJob.class);
	...
	protected void executeInternal(JobExecutionContext paramJobExecutionContext)
		...
		try {
			...
			String businessid = jobDataMap.getString("gmnum");
			HomeService mstationService = (HomeService) jobDataMap.get("HomeService");
			mstationService.updatePoolJF(businessid);
